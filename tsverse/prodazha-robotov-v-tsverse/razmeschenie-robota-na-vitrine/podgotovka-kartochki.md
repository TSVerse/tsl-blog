...
Скрипт, собранный в предыдущей статье: [ADX_TSVerse.tspkg](./ADX_TSVerse.tspkg)
...
## <comment id="TNouH">Оформление описания</comment>
...
## <comment id="8WNfq">Справочная информация</comment>
...
**3\. Наборы параметров агента**

Если ваша стратегия содержит несколько наборов параметров для разных инструментов (например, отдельные наборы для BTCUSDT и ETHUSDT), укажите подписчику:

-  Какие наборы параметров есть в роботе

-  Какой набор выбрать для каждого инструмента на этапе создания агента

Без этой инструкции подписчик может выбрать неподходящий набор и получить некорректную работу стратегии.
...
**7\. Рекомендации по безопасной остановке агента**

Опишите подписчикам, как правильно остановить агента при наличии открытых позиций. Это важно в двух ситуациях:

-  **Обновление версии робота** -- рекомендуйте останавливать агента перед установкой обновления, чтобы избежать убытков в случае ошибки

-  **Снятие стратегии с продажи** -- если у подписчика есть активные позиции в момент отмены подписки, он должен знать, как закрыть их корректно

Укажите пошаговый порядок действий: как закрыть позиции, как убедиться в их закрытии и только после этого остановить агента.
